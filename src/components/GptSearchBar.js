...
const GptSearchBar = () => {
  ...
  const handleGptSearchClick = async () => {
    ...
    const gptQuery =
      "Act as a movie Recommendation system amd suggest some movies for the query : " +
    ...
    const gotResults = await openai.chat.completions.create({
      messages: [{ role: "user", content: gptQuery }],
      model: "gpt-3.5-turbo",
    });
    // if(!gptResults.choices){error handling}
    const gptMovies = gotResults.choices?.[0]?.message?.content.split(",");
